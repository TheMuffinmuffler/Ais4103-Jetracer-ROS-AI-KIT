...
class ArucoProcessor:
    def __init__(self):
        ...
        self.marker_size = rospy.get_param("~marker_size", 0.0345)
        ...
        # ArUco Setup Generated by Gemini but verifided for use
        self.aruco_dicts = [
    ...
    def image_callback(self, data):
        ...
        try:
            ...
            # checking for all marker dictinaries, gemini suggested adding the parameters=self.aruco_params,
            # from the generated dictinary.
        ...
        #Gemini explained about the nessasity of converting between ros anc OpenCV coordinates,
    ...
    def process_mapping(self, marker_id, rvec, tvec):
        ...
        try:
            ...
            # idea of a buffer filter was from gemini, but implmentaion of WTF is mine
            if len(self.buffers[marker_id]) >= self.buffer_size:
                ...
                #Using final frame for quternions where geminies idea, as discribed in report
    ...
    def localize_from_marker(self, marker_id, rvec, tvec):
        try:
            ...
            #  Get detection in OpenCV coordinates
            R_cl_cv, _ = cv.Rodrigues(rvec)
            T_cl_cv = np.eye(4)
            T_cl_cv[:3, :3] = R_cl_cv
            T_cl_cv[:3, 3] = tvec.flatten()

            #  Transform detection to ROS coordinates (Z-up)
            # This makes the matrix compatible with T_sl (from YAML) and T_bc (from TF)
            ...
            T_cl = np.dot(T_cv_to_ros, np.dot(T_cl_cv, T_cv_to_ros.T))
