...
class ArucoMapper:
    ...
    def info_callback(self,msg):
        if self.camera_matrix is None and msg.K[0] !=0:
            self.camera_matrix = np.array(msg.K).reshape((3,3))
            self.dist_coeffs = np.array(msg.D)
            print("\n--- Camera Calibration Received! Ready for 3D Math ---\n")
    ...
    def callback(self,data):
        ...
        if ids is not None:
            ...
            if self.camera_matrix is not None:
                ...
                print("Found Marker [{}], but waiting for Camera Calibration...".format(ids.flatten()))
