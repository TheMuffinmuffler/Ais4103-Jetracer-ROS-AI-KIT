...
private:
  ...
  std::vector<float> dist_to_wall; // distance to nearest obstacle for each cell
...
*/ 
    double start_wx = 14.6693248749;
    double start_wy = 8.143907547; // 3 
    double goal_wx  = -4.00995779037;
    double goal_wy  = -0.996777892113 ;// 0
...
// generated by Claude 
// ---------------------------- isCellFreeInflated ----------------------------
// cell is free only if it is farther than inflation_radius from obstacles
bool isCellFreeInflated(int gx, int gy)
{
  ...
  float inflation_radius = 5.0f;   // in cells, first value to test
  return dist_to_wall[toIndex(gx, gy)] >= inflation_radius;
}


// code generated by Claude 
// ---------------------------- wallPenalty ----------------------------
// centering penalty: every cell gets a cost based on distance to walls
int wallPenalty(int gx, int gy)
{
  if (gx < 0 || gy < 0 || gx >= width || gy >= height)
    return 0;

  float d = dist_to_wall[toIndex(gx, gy)];
  float max_dist = 25.0f;   // cells
  float weight   = 3.0f;    // tune later if needed

  if (d >= max_dist)
    return 0;

  return (int)(weight * (max_dist - d));
}


// code generated by Claude 
// ---------------------------- computeDistanceField ----------------------------
// compute distance from every cell to the nearest obstacle, once per map
void computeDistanceField()
{
  int total = width * height;
  dist_to_wall.assign(total, std::numeric_limits<float>::max());

  std::queue<int> q;

  // initialize all obstacle cells with distance 0
  for (int y = 0; y < height; y++)
  {
    for (int x = 0; x < width; x++)
    {
      ...
      {
        int idx = toIndex(x, y);
        dist_to_wall[idx] = 0.0f;
        q.push(idx);
      }
    }
  }

  // 8-connected propagation
  const int dx8[8] = { 1, -1,  0,  0,  1,  1, -1, -1 };
  const int dy8[8] = { 0,  0,  1, -1,  1, -1,  1, -1 };
  const float dc[8] = { 1.0f, 1.0f, 1.0f, 1.0f, 1.414f, 1.414f, 1.414f, 1.414f };
  ...
  while (!q.empty())
  {
    ...
    for (int d = 0; d < 8; d++)
    {
      ...
      float nd = dist_to_wall[cur] + dc[d];

      if (nd < dist_to_wall[ni])
      {
        dist_to_wall[ni] = nd;
        q.push(ni);
      }
  ...
  ROS_INFO("Distance field computed.");
}
